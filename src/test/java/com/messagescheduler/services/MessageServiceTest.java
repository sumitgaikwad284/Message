...
class MessageServiceTest {
	...
	void saveMessageTesting() throws SQLErrorException {
		...
		Request request = new Request("Hii","9359370619","2022-07-30T15:45:20");
	...
	@Test
	void saveMessageNullTesting() throws SQLErrorException{
		String actucal = "";
		String expected = "Error in inserting message!!";
		try {
			when(messageDao.insertMessageInDB(any(), any())).thenThrow(new SQLErrorException("Error in inserting message!!"));
			int res = messageService.saveMessage(null, null);
			assertThat(1).isEqualTo(res);
		}
		catch(SQLErrorException e) {
			actucal = e.getErrorMessage();
		}
		assertThat(actucal).isEqualTo(expected);
	}
}
